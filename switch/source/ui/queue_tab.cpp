        ...
        for (const auto& job : jobs) {
            ...
            cell->registerClickAction([job](brls::View*) {
                ...
                if (job.status == "queued" || job.status == "interrupted") {
                    ...
                    dialog->addButton("hints/ok"_i18n, [job]() {
                        brls::sync([job] { Session::instance().claimAndInstall(job); });
                    });
                ...
                dialog->addButton("hints/ok"_i18n, [id = job.id]() {
                    brls::sync([id] { Session::instance().cancelJob(id); });
                });
                dialog->addButton("hints/cancel"_i18n, []() {});
                dialog->open();
                return true;
