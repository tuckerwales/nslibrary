...
Forgot the admin password? Open a terminal on the container (Coolify: **Terminal**) and run `node dist/main.js reset-password` as the `node` user. It signs out every session.
